...
import { REGEX_EMAIL } from "../constant/app.constant";

// describe: tiêu đề, gom các case (trường hợp)
// it: cụ thể một case
...
describe("Auth Service", () => {
  ...
  it("Case: 1: Trường hợp đăng ký thành công với thông tin hợp lệ", async () => {
    ...
    //giả lập dữ liệu đầu ra của 2 hàm findFist và create VỚI TRƯỜNG HỢP THÔNG TIN HỢP LỆ
    ...
    await prisma.users.create.mockResolvedValue({
      ...
      goole_id: null,
      face_app_id: null,
      created_at: "2025-01-31T01:24:35.000Z",
      updated_at: "2025-01-31T01:24:35.000Z",
      role_id: 2,
    });
  ...
  it("Case: 2: Trường hợp đăng ký, email đã tồn tại, cần phải báo looix", () => {
    // console.log(`Case 2`)
  });
});
